...
namespace ScratchCardAsset
{
	public class ScratchCard : MonoBehaviour
	{
        ...
        private void Awake()
        {
			...
			uicontrols.UIControl.Rascar.performed += x => Rascar();
			uicontrols.UIControl.NoRascar.performed += x => NoRascar();
			//uicontrols.UIControl.MousePosition.performed += x => MousePosition();
		}
        void Start()
		{
			
			MainCamera = GameObject.Find("UI Camera").GetComponent<Camera>();
			Init();
		}
		...
		void Update()
		{
			...
			if(rascando)
				cardInput.Rascar(mousePosition);
		}
